    ...
    if (event->key() == Qt::Key_Delete)
    {
        if (selectedObjects.size() > 0)
        {
            for (int i=0; i<selectedObjects.size(); ++i)
            {
                ...
                {
                    if (m_curSplineIdx == targetId) m_curSplineIdx = -1;
...
void GLScene::addCurveItem(int cid)
{
    ...
    curveItem->setFlags(QGraphicsItem::ItemIsSelectable);
    addItem(curveItem);
    curveItems.push_back(curveItem);

    for (int i=0; i<curveItems.size()-1; ++i)
    {
        curveItems[i]->setSelected(false);
    }
    curveItems.back()->setSelected(true);

}

void GLScene::updateCurveItem(SplinePathItem* curveItem)
{
    if (!curveItem->segmentsItem)
    {
        curveItem->segmentsItem = new QGraphicsPathItem(curveItem);
    }
...
/*void GLScene::handleSelectionChanged()
{
    for (int i=0; i<curveItems.size(); ++i)
    {
        if ((m_sketchmode == ADD_CURVE_MODE && m_curSplineIdx==curveItems[i]->spline->idx) ||
            (m_sketchmode == IDLE_MODE && curveItems[i]->isSelected()))
        {
            m_curSplineIdx = curveItems[i]->spline->idx;
            for (int j=0; j<pointItems.size(); ++j)
            {
                ControlPoint& cpt = *pointItems[j]->point;
                if (std::find(cpt.connected_splines.begin(),
                              cpt.connected_splines.end(), m_curSplineIdx) != cpt.connected_splines.end())
                {
                    pointItems[j]->setSelected(true);
                } else
                {
                    pointItems[j]->setSelected(false);
                }
            }
            curveItems[i]->setPen(QPen(Qt::blue, 3, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));

        } else
        {
            curveItems[i]->setPen(QPen(Qt::red, 3, Qt::SolidLine, Qt::RoundCap, Qt::RoundJoin));
        }
        updateCurveItem(curveItems[i]);
    }
}
